...
export const TopNavigation: React.FC<TopNavigationProps> = ({
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-6 py-3 bg-slate-950/60 backdrop-blur-xl border-b border-white/10 shadow-2xl">
      ...
      <div className="flex items-center space-x-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-outfit font-bold text-lg tracking-wider text-white">
            EXPLORADOR <span className="text-cyan-400">CÓSMICO</span>
          </span>
          <span className="text-[10px] font-mono uppercase tracking-widest px-1.5 py-0.5 rounded bg-cyan-500/10 text-cyan-300 border border-cyan-500/20">
            NASA / ESA GRADE
          </span>
        </div>
      ...
      <nav className="flex items-center space-x-1 bg-black/40 p-1 rounded-full border border-white/10" aria-label="Navegación principal del observatorio">
        <button
          onClick={() => onModeChange('solar')}
          className={`flex items-center space-x-2 px-4 py-1.5 rounded-full text-xs font-medium transition-all duration-300 ${
            currentMode === 'solar'
      ...
      <div className="flex items-center space-x-2">
        ...
        <button
          ...
          className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-orange-500/20 hover:bg-orange-500/30 text-orange-300 border border-orange-500/30 text-xs font-medium transition-all"
        >
          <span className="w-2 h-2 rounded-full bg-orange-500 animate-ping" />
          <span>Gargantua GRRT</span>
        </button>
        ...
        {profile.id === 'profesor' && (
          <button
            ...
            className="p-2 rounded-lg bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 transition-colors"
          >
        ...
        <button
          ...
          className="flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-gradient-to-r from-cyan-500/20 to-blue-500/20 hover:from-cyan-500/30 hover:to-blue-500/30 border border-cyan-500/40 text-cyan-300 text-xs font-semibold transition-all shadow-md"
        >
